...
# # Function 2: Generates random data for QEBD ----------------------------

# n: Number of sample data to be generated
# V: A "m x m" diagonal matrix, with main effects placed at diagonal and interaction effects placed at off-diagonal positions.
...
# # Function 3: Generates random data for QELR ----------------------------
...
GenData_QELRcommon <- function(beta, gamma, n, p){
  nbeta <- length(beta)
...
GenData_QELRlinear <- function(beta, gamma, n, p){
  ngamma <- length(gamma)
  nbeta <- length(beta)
  X<- cbind(1, sapply(1:(nbeta-1), function(i) rnorm(n*p))) #
  Y<- matrix(0,n,p)
...
# Function 4: Parameter estimation  ---------------------------------------
...
EstQELR <- function(dat, method, ... ){
  #dat must contains all variables, including a id column
  args <- list(...)
  
  if(method=="logit"){
    fit <- glm(y~.-1-id, family=binomial, data=dat)
    est <- coef(fit)
    V <- vcov(fit)
    se <- diag(V) %>% sqrt
